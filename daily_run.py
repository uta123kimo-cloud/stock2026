...
SYMBOLS = list(dict.fromkeys([
    ...
    "3491","2367","6285","6190","3062","2419","2314","3305","3105","2312","8086",
...
class _RegimeEngine:
    ...
    def run(self, market_data: dict, today: str) -> dict:
        try:
            ...
            close = float(last["Close"]); ma60 = float(last["MA60"])
            ...
            ma60_sl = (ma60_s[-1] - ma60_s[-6]) / (ma60_s[-6] + 1e-9) if len(c) > 65 and not math.isnan(ma60_s[-6]) else 0.0
            ...
            if not any(h["month"] == curr_month for h in old_hist):
                old_hist.append({"month": curr_month,
...
def _v4_signal(pvo, vri, slope_z, sc, mu, sigma, pvo_c, pvo_a, vri_d, vol_ratio):
    ...
    sig_q = (1.3 if vol_ratio >= 2.0 else 1.1 if vol_ratio >= 1.2 else
...
def _v12_features(df) -> dict:
    ...
    try:
        ...
        i=-1
        return {"bb_width":float(bb_w[i]),"rsi_14":float(rsi[i]),"ma20_slope":float(ma20sl[i]),
                "price_mom_5":float(pm5[i]),"price_mom_20":float(pm20[i]),"price_mom_60":float(pm60[i]),
                "rs_vs_mkt_5":0.0,"mkt_excess_z":float(ymad[i]),"inst_cum_10":float(ic10[i]),
                "vol_pvo":float(vpvo[i]),"vol_pvo_sq":float(vpvoq[i]),"persist_count":float(persist[i]),
                "atr_regime":float(atr_reg[i]),"vri":float(vri[i]),"close_ma5_r":float(cma5r[i]),
                "inst_pvo":float(ipvo[i]),"inst_x_price":float(ixp[i]),"high52w_dist":float(h52d[i]),
    ...
    for cand in cands[:12]:
        ...
        try:
            ...
            if sym in old_pos:
                ...
                exit_sig=_v12_exit(old,ev_soft,slope_now,days,curr_ret)
            else:
                days=0; entry_price=close; curr_ret=0.0; action="進場"; exit_sig="—"

            ev_tier=("⭐核心" if ev_soft>=0.050 else "🔥主力" if ev_soft>=0.030 else
                     "📌補位" if ev_soft>=0.020 else "")

            positions.append({
                "symbol":sym,"path":best_path,"ev":round(ev_soft*100,2),"ev_tier":ev_tier,
                "action":action,"exit_signal":exit_sig,"quality":quality,"days_held":days,
                "curr_ret_pct":round(curr_ret*100,2),"entry_price":round(entry_price,2),
