"""
╔══════════════════════════════════════════════════════════════╗
║   資源法 AI 戰情室  v4.1  —  純展示層 (Display-Only)         ║
║   架構：Precompute + GitHub Storage + Streamlit Render       ║
║   資料來源：GitHub Raw JSON (每日自動更新)                    ║
║                                                              ║
║   v4.1 修正：                                                ║
║   [V4.1-01] 自選股快覽：V4持倉 + V12持倉 + watchlist合并     ║
║   [V4.1-02] 自選股快覽增加「持天/停利/停損」欄位              ║
║   [V4.1-03] 刪除「交易歷史」Tab頁                            ║
║   [V4.1-04] V4頁面增加停利/停損價欄位                        ║
║   [V4.1-05] V12部位監控增加候選5檔                           ║
║   [V4.1-06] 大盤指數修正 + 歷史圖表說明更明確                ║
║   [V4.1-07] 重設為淡色系 UI                                  ║
╚══════════════════════════════════════════════════════════════╝
"""
...
[data-testid="stSidebar"] {
    ...
    box-shadow: 2px 0 12px rgba(30,60,110,0.06);
}
...
.hq-badge {
    ...
    font-size:0.72rem; font-weight:700; font-family:var(--mono); letter-spacing:1px;
}
...
.pill {
    ...
    font-family:var(--mono); border:1px solid; margin:1px;
}
...
.data-table td {
    padding:9px 12px; border-bottom:1px solid rgba(59,130,246,0.07);
    color:var(--text);
}
...
.candidate-divider { 
    ...
    border-top:2px dashed rgba(217,119,6,0.3)!important;
}
.candidate-divider td {
...
/* ── Regime 條 ── */
.regime-bar {
    display:flex; height:38px; border-radius:var(--radius);
...
[data-testid="stTab"] button {
    color:var(--text-mid)!important; font-family:var(--sans)!important;
...
.stSelectbox label, .stMultiSelect label, .stSlider label {
    color:var(--text-mid)!important; font-size:0.82rem!important;
}
.stTextInput input { background:var(--panel)!important; color:var(--text)!important; border:1px solid var(--border2)!important; border-radius:var(--radius-sm)!important; }
...
def _mock_v12():
    ...
    # 候選股（非正式持倉）
    candidates = []
    cand_syms = ["5274","3131","4763","6148","3293"]
    for s in cand_syms:
...
:root{{
    --bg:#f0f4f8; --bg2:#e8edf4; --bg3:#dce4ef; --panel:#ffffff;
...
# ══════════════════════════════════════════════════════════════
# [V4.1-04] Section 1: V4（增加停利/停損欄位）
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# [V4.1-05] Section 2: V12.1（持倉 + 候選5檔）
# ══════════════════════════════════════════════════════════════
...
def render_v12_section(v12: dict):
    ...
    candidates = v12.get("candidates",[])[:5]  # 最多5檔候選
    stats      = v12.get("stats",{})
...
# ══════════════════════════════════════════════════════════════
# [V4.1-06] Section 3: Regime（大盤修正 + 圖表說明）
# ══════════════════════════════════════════════════════════════
...
def render_regime_section(regime: dict, market: dict):
    ...
    # [V4.1-06] 大盤指數：優先使用 regime 本身的 index_close/chg
    ...
    # [V4.1-06] 歷史圖表說明更明確：雙軸（Regime機率 + 指數走勢）
    ...
    if history:
        ...
        fig = make_subplots(
            ...
            shared_xaxes=True,
            subplot_titles=[
                "月末 Regime 三態機率分佈（熊/震盪/牛，各月合計=100%）",
                "TAIEX 加權指數月收盤走勢"
            ],
        ...
        if "index_close" in df_r.columns:
            ...
            fig.add_scatter(
                x=df_r["month"], y=df_r["index_close"],
        ...
        fig.update_layout(
            ...
            legend=dict(
                font=dict(color="#475569"), orientation="h",
                y=1.08, x=0, bgcolor="rgba(255,255,255,0.8)",
                bordercolor="rgba(59,130,246,0.2)", borderwidth=1
            ),
            xaxis=dict(gridcolor="rgba(59,130,246,0.1)", color="#94a3b8"),
            yaxis=dict(gridcolor="rgba(59,130,246,0.1)", color="#94a3b8",
                       range=[0,100], ticksuffix="%", title="機率"),
            xaxis2=dict(gridcolor="rgba(59,130,246,0.1)", color="#94a3b8"),
            yaxis2=dict(gridcolor="rgba(59,130,246,0.1)", color="#94a3b8",
                        title="指數點位"),
        )
        # 圖表說明標題顏色
        ...
        # 附加文字說明
        st.caption("📌 上圖說明：每月底系統對大盤狀態估計機率（熊/震盪/牛），機率越高代表該制度越強。牛市主用路徑45，震盪主用路徑423，熊市縮手。")
        st.caption("📌 下圖說明：TAIEX 加權指數月底收盤點位走勢，反映系統運行期間的真實大盤環境。")
...
def main():
    ...
    <div class="hq-header">
        <div>
            <div class="hq-title">📊 資源法 AI 戰情室 <span style="font-size:0.85rem;color:#94a3b8;">v4.1</span></div>
    ...
    if ai_btn:
        with st.spinner("🤖 Gemini 分析中..."):
            summary = call_gemini(
                build_dashboard_prompt(v4, v12, regime, market),
